...
USAGE = """
    generate-gwt-proxy [--all] [--non-recursive] [--pkg=PKG]
                 [--oldpkg=OLDPKG --newpkg=NEWPKG] [DOMAINCLASS]
                
where
    PKG         = The target package to put the proxy classes in. By default,
                  each proxy class goes in the same package as its domain class.
    ...
    DOMAINCLASS = The fully qualified name of the domain class to generate
                  a proxy for. Ignored if --all is used.
"""

target(default: 'Generates proxy classes for one or more domain classes.') {
    depends(compile, classpath, loadApp, bootstrap, parseArguments)

    // Check that the "grails.plugin.gwt.requestfactory.package.transforms" setting is the correct type.
    if (!buildConfig.grails.plugin.gwt.requestfactory.package.transforms instanceof Map) {
        println "The 'grails.plugin.gwt.requestfactory.package.transforms' build setting must be a map."
        exit(1)
    }
    ...
    def domainClasses
    if (argsMap['all']) {
        def response = confirmInput('Are you sure you want to generate proxies for all domain classes?')
        if (!response) {
            return 0
        } else {
    ...
    // application context.
    def generator = grailsApp.mainContext.gwtProxyGenerator

    // If we have a target package, initialise the generator's package
    // transformations.
    if (targetPackage) {
        generator.packageTransforms = [(replacePackage): targetPackage]
    } else if (buildConfig.grails.plugin.gwt.requestfactory.package.transforms) {
        generator.packageTransforms = buildConfig.grails.plugin.gwt.requestfactory.package.transforms
    }
    ...
    event('StatusFinal', ['Successfully created the proxy under src/java'])
}
